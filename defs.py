...
def mp_cv_score(X, y, train_index, test_index, model, transformer):
    ...
    print('Fold {}/{}. Score: {}'.format(i+1,n,score))
    return score


class stacked_model:
    def __init__(self, stacking_model, models):
        self.stack = stacking_model
        self.models = models
        
    def fit(self, X_sets, y_set):
        p_train = np.empty([len(X_sets[0]),0])
        for i in range(0, len(self.models)):
            self.models[i].fit(X_sets[i], y_set)
            p = self.models[i].predict_proba(X_sets[i])
            p_train = np.concatenate((p_train, p), axis=1)
        self.stack.fit(p_train, y_set)
        
                    
    def score(self, X_sets, y_set):
        p_test = np.empty([len(X_sets[0]),0])
        for i in range(0, len(self.models)):
            p = self.models[i].predict_proba(X_sets[i])
            p_test = np.concatenate((p_test, p), axis=1)
        score = self.stack.score(p_test, y_set)
        return score
    
    def validate(self, X_sets, y_set, train_test_index):
        self.fit([X.loc[train_test_index[0]] for X in X_sets], y_set.loc[train_test_index[0]])
        score = self.score([X.loc[train_test_index[1]] for X in X_sets], y_set[train_test_index[1]])
        return score
    
    
    def cross_validate(self, X_sets, y_set, n_iter, n_jobs=1):
        kf = KFold(n_splits=n_iter).split(X_sets[0])
        with mp.Pool(n_jobs) as p:  
            f = partial(self.validate, X_sets, y_set)
            scores = p.map(f, list(kf))
